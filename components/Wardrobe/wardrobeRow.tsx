...
import { WardrobeWoodItem, AccessoryItem } from "@/store/sumStore";
...
type Props = {
  ...
  updateRowData: (data: WardrobeWoodItem) => void;
};

export const WardrobeRow = (props: Props) => {
  const { data, updateRowData, id, deleteRow } = props;

  // Initialize state with first item
  const [cost, setCost] = useState(data[0].value);
  const [finishName, setFinishName] = useState(data[0].name);
  ...
  useEffect(() => {
    updateRowData({
        ...
        length,
        height,
        totalSqft: Math.round(height * length),
        total: grandTotal
    });
  }, [grandTotal, finishName, length, height, cost, updateRowData, id]);
...
type accessories = {
  ...
  updateRowData: (item: AccessoryItem) => void;
  id: number;
  isKitchen?: boolean;
...
export const RowAccessories = (props: accessories) => {
  ...
  const initializeState = React.useCallback((selectedItem: string, state: "Brand" | "Size") => {
  ...
  }, [grandTotal, item, selectedBrand, selectedSize, uom, price, updateRowData, id]);
