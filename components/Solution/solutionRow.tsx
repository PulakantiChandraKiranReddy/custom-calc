...
import { ServiceItem } from "@/store/sumStore";

interface Props {
  deleteRow: (id: number) => void;
  updateRowData: (data: ServiceItem) => void; 
  id: number;
}

const SolutionRow: React.FC<Props> = ({ deleteRow, updateRowData, id }) => {
  ...
  useEffect(() => {
    ...
    updateRowData({
        id,
        name: activeRow?.name || "",
        specification: activeRow?.specification || "N/A",
        description: activeRow?.description || "",
        rate: activeRow?.rate || 0,
        quantity,
        unit: activeRow?.unit,
        total
    });
  }, [activeRow, quantity, id, updateRowData]);
